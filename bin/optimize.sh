...
# -------------------------------------------------------------------------- #
# optimize.sh - compacta o indice no servidor
# -------------------------------------------------------------------------- #
...
#		 -p, --producao		Efetua otimizacao na producao
#		 -P, --prova		Efetua otimizacao na homologacao
#		 -V, --version		Mostra a versao
#       Saida:  Indice compactado
#		Codigos de retorno:
#		 0 - Operacao ok
...
#     Chamada:  optimize.sh [-h|-V|--changelog]|[-d N][-e][-p|-P] <ID_INDEX>
#     Exemplo:  optimize.sh -d 132 bioetica
# Objetivo(s):  Organizar o indice estruturalmente
# Comentarios:  A otimizacao do indice inicia a operacao fazendo um commit
...
OPCOES:
 ...
 -p, --producao     Efetua otimizacao no servidor de producao
 -P, --prova        Efetua otimizacao no servidor de prova (homologacao)
 -V, --version      Exibe a versao corrente do comando e para a execucao

PARAMETROS:
 PARM1   Identificador do indice a otimizar
"
...
echo "[optime]  1         - Index OPTIMIZE"

# -------------------------------------------------------------------------- #
# Garante que a instancia esta definida (sai com codigo de erro 2 - Syntax Error)
if [ -z "$PARM1" ]; then
	echo "[optime]  1.01      - Erro na chamada falta o parametro 1"
	echo
...
#                                        1234567890123456789012345
[ $N_DEB -ne 0 ]                    && echo "[optime]  0.00.01   - Testa se ha tabela de configuracao"
[ ! -s "$PATH_EXEC/tabs/iAHx.tab" ] && echo "[optime]  1.01      - Tabela iAHx nao encontrada" && exit 3
...
#                                        1234567890123456789012345
[ $N_DEB -ne 0 ] && echo "[optime]  0.00.02   - Testa se o indice eh valido"
IDIDX=$(rdANYTHING $PARM1)
[ $? -eq 0 ]     && INDEX=$(rdINDEX $IDIDX)
[ -z "$INDEX" ]  && echo "[optime]  1.01      - PARM1 nao indica um indice valido" && exit 4
...
echo "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" >  $PATH_IAHX/otimize.xml
echo "<optimize/>"                                >> $PATH_IAHX/otimize.xml
mv $PATH_IAHX/otimize.xml $PATH_IAHX/optimize.xml
[ $N_DEB -gt 1 ] && cat   $PATH_IAHX/optimize.xml

# Discovery where index are instaled
#     1234567890123456789012345
echo "[optime]  2         - Prepara o otimizacao de indice"
echo "[optime]  2.01      - Descobre em qual servidor esta o indice"
for instance in $(ls ${PATH_IAHX}/../instances/)
do
	[ $N_DEB -gt 1 ] && echo "[optime]  2.01.0$instance   - Varrendo server numero $instance"
...
if [ "$SERVERNO" = "" ]; then
	echo
	echo "[optime]  2.02      - Fatal: Index are not available on intances servers"
	echo
	exit 4
fi

# Concatenate default 898 to server number. ex. 8981 to server1 parameter resulting 8981
PORT="898${SERVERNO}"

[ $N_DEB -gt 1 ] && echo "[optime]  2.02.00   - PORT = $PORT"

echo "[optime]  2.02      - Otimize index ${INDEX} on iahx-server ${SERVERNO} in ${SERVIDOR} at $HRINI"

[ $N_DEB -ne 0 -a "$SERVIDOR" != "." ] && echo "java -jar ${PATH_IAHX}/postXML.jar http://${SERVIDOR}:$PORT/${INDEX}/update ${PATH_IAHX}/optimize.xml"
# Se nao eh execucao FAKE nem o servidor inexistente (.) manda otimizar
if [ $FAKE -eq 0 -a "$SERVIDOR" != "." ]; then
	java -jar ${PATH_IAHX}/postXML.jar http://${SERVIDOR}:$PORT/${INDEX}/update ${PATH_IAHX}/optimize.xml
	RSP=$?; [ "$NOERRO" = "1" ] && RSP=0
	source checkerror $RSP "optimize fail for index $INDEX on iahx-server $SERVERNO in ${SERVIDOR}: $(date '+%Y-%m-%d %H:%M:%S')"
fi
...
20140312 Garantia de existencia de optimize.xml
SPICEDHAM
